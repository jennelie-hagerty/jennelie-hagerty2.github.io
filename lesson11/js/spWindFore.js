...
const apiURL =
  "https://api.openweathermap.org/data/2.5/weather?id=5607916&units=imperial&appid=262da0a1a160bae2ae592c408e28775d";
...
fetch(apiURL)
  ...
  .then((jsObject) => {
    ...
    document.getElementById("spcurrent-temp").textContent = Math.round(jsObject.main.temp);
    document.getElementById("spwindspeed").textContent = Math.round(jsObject.wind.speed);
    document.getElementById("sphumidity").textContent = Math.round(jsObject.main.humidity);
    document.getElementById("spcondition").textContent = (jsObject.weather[0].main);
  });

  const temp = document.getElementById("spcurrent-temp").innerHTML;
  const speed = document.getElementById("spwindspeed").innerHTML;
  ...
  if ((temp <= 50) && (speed > 3)) {
      ...
      document.getElementById("spwindchill").innerHTML = answer;
  } else {
    document.querySelector(".spwinC").innerHTML = "Windchill: N/A";
  }
  ...
  .then((forecast) => {
    ...
    for (let x in forecast.list) {
      ...
      if (forecast.list[x].dt_txt.includes("18:00:00")) {
        ...
        document.querySelector(".spfivedaycontainer").appendChild(dayBox);
      }
  ...
  .then(function (jsonObject) {
    ...
    for (let i = 0; i < towns.length; i++) {
      if (i == 0) {
        ...
        let sph2 = document.createElement("h2");
        sph2.classList.add("sph2");
        sph2.innerHTML = `Events`;
        event.appendChild(sph2);
        
        let soda = document.createElement('p');
        soda.innerHTML = `${towns[i].events}`;
        soda.classList.add("soda");
        event.appendChild(soda);
        
        document.getElementById("towneventsss").appendChild(event);
      }
      }});